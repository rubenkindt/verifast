...
// Based on Gotsman et al., "Local Reasoning for Storable Locks and Threads", APLAS 2007.

// These are like mutexes, except that:
// 1) Acquiring a leaky mutex asserts the presence of, but does not consume, the mutex chunk
// 1) Releasing a leaky mutex leaves nothing behind
// 2) Disposing a leaky mutex requires that the mutex be held by the thread

// This mutex is called leaky because it can be (ab)used to leak resources.

struct leaky_mutex;
typedef struct leaky_mutex *leaky_mutex;

/*@

predicate leaky_mutex(struct leaky_mutex *mutex; predicate() p);

predicate leaky_mutex_held(struct leaky_mutex *mutex, predicate() p, int threadId);
...
struct leaky_mutex *create_leaky_mutex();
    ...
    //@ ensures result != 0 &*& leaky_mutex(result, p);
    
void leaky_mutex_acquire(struct leaky_mutex *mutex);
    //@ requires [?f]leaky_mutex(mutex, ?p);
    //@ ensures [f]leaky_mutex(mutex, p) &*& leaky_mutex_held(mutex, p, currentThread) &*& p();

void leaky_mutex_release(struct leaky_mutex *mutex);
    //@ requires leaky_mutex_held(mutex, ?p, currentThread) &*& p();
    //@ ensures true;

void leaky_mutex_dispose(struct leaky_mutex *mutex);
    //@ requires leaky_mutex(mutex, ?p) &*& leaky_mutex_held(mutex, p, currentThread);
    //@ ensures true;

struct leaky_mutex_cond;
typedef struct leaky_mutex_cond *leaky_mutex_cond;

//@ predicate leaky_mutex_cond(struct leaky_mutex_cond *cond; struct leaky_mutex *mutex);

struct leaky_mutex_cond *create_leaky_mutex_cond();
    //@ requires exists<struct leaky_mutex *>(?mutex);
    //@ ensures result != 0 &*& leaky_mutex_cond(result, mutex);

void leaky_mutex_cond_dispose(struct leaky_mutex_cond *cond);
    //@ requires leaky_mutex_cond(cond, _);
    //@ ensures true;

void leaky_mutex_cond_wait(struct leaky_mutex_cond *cond, struct leaky_mutex *mutex);
    //@ requires [?fc]leaky_mutex_cond(cond, mutex) &*& [?fm]leaky_mutex(mutex, ?p) &*& leaky_mutex_held(mutex, p, currentThread) &*& p();
    //@ ensures [fc]leaky_mutex_cond(cond, mutex) &*& [fm]leaky_mutex(mutex, p) &*& leaky_mutex_held(mutex, p, currentThread) &*& p();

void leaky_mutex_cond_signal(struct leaky_mutex_cond *cond);
    //@ requires [?fc]leaky_mutex_cond(cond, ?mutex) &*& leaky_mutex_held(mutex, ?p, currentThread);
    //@ ensures [fc]leaky_mutex_cond(cond, mutex) &*& leaky_mutex_held(mutex, p, currentThread);
